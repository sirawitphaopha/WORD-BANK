# โจทย์: สกัดคำงามจากคลังเดิม 654 วลี

งานนี้ทำ **อย่างเดียว** คือ **สกัดคำงามที่ซ่อนอยู่ในวลี** ไม่ต้องจัดหมวด ไม่ต้องตรวจกิ่ง

## ไฟล์
`docs/oldwords/extract/in.jsonl` — บรรทัดละ 1 วลี `{"n":เลขบรรทัด,"t":"ตัววลี","len":ความยาว,"cat":["หมวดที่มันอยู่"]}`

## 🚨 กฎเหล็กที่ห้ามผิดเด็ดขาด
1. **คำที่สกัดต้องเป็นข้อความ "ต่อเนื่องกัน" ที่ตัดออกมาจากวลีเดิมเท่านั้น**
...
4. **วลีเดิมไม่ถูกแตะ** — การสกัดคือการ "เพิ่มคำใหม่" วลีเดิมยังอยู่ครบเสมอ
5. **ก้ำกึ่งให้สกัดไว้ก่อน** — เจ้าของคลังเป็นคนกดเลือกและลบเอง

## 🎯 เกณฑ์ "คำงาม" (กฎข้อ 3ข ของคำสั่ง AI ที่เว็บใช้จริง)
ดูกฎเต็มด้านล่าง สรุปสั้น: คำที่เปลี่ยน **ข้อมูล → ประสบการณ์**
4 หน้าที่: ปลุกภาพจำ · แสดงอารมณ์ผ่านร่างกาย · จับเฉดอารมณ์ตรงจุด · คุมจังหวะฉาก
6 รูปแบบที่พบบ่อย: คำซ้อนที่มีเนื้อสัมผัส · กริยาที่ติดภาพ/ติดเสียง · คำเปรียบ · สำนวนไทย · วลีที่วาดภาพครบ · คำทับศัพท์ที่สะกดยาก
🚫 ข้าม: คำทั่วไปที่นักเขียนนึกออกเอง · คำเชื่อม/สรรพนาม/คำลงท้าย · ชื่อเฉพาะ · คำไร้สีสัน

## รูปแบบคำตอบ
เขียน `docs/oldwords/extract/outK.jsonl` (K = เลขก้อนของคุณ) บรรทัดละวลี **เฉพาะวลีที่สกัดได้**
(วลีที่ไม่มีอะไรให้สกัด ข้ามไปเลย ไม่ต้องเขียนบรรทัดเปล่า)

```json
{"n":12,"t":"วลีเดิมเป๊ะ ๆ","ex":[{"w":"คำที่สกัด","meaning":null},{"w":"อีกคำ","meaning":"ความหมายไม่เกิน 60 ตัวอักษร"}]}
```

- `meaning` ใส่เฉพาะคำวรรณกรรม/คำโบราณ/สำนวนที่ควรอธิบาย · คำธรรมดาใส่ `null`
- ไม่ต้องใส่หมวด ไม่ต้องใส่กิ่ง — งานนั้นทำทีหลัง
