...
## 5. ไฟล์ + เครื่องมือ
- 🚨🚨 **เวิร์กโฟลว์/agent ใช้แค่ 3-5 ตัวพอ ห้าม 30+ (พี่กันตั้ง 23 ก.ค. 2569 · "เปลือง token มาก"):** ตอนใช้ Workflow/subagent หลายตัว ให้จำกัด **3-5 agent** · **ห้ามแตกงาน verify เป็น agent ย่อยตัวละ item** (เช่น ตรวจคำทีละคำ = agent ละคำ → 30+ ตัว) · ให้ทำ scan หลายมุม 3 ตัว แล้ว **สังเคราะห์/ตัดสินในโค้ดหรือในตัว agent เดียว** ไม่แตกเป็น agent · เคยพลาด: รอบตรวจคำเดิม→กิ่งใหม่ แตก verify 30+ ตัว พี่กันสั่งหยุดกลางคัน
