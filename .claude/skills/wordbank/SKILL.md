...
- **ตัวจัดคำ = ระบบ AI สลับหลายเจ้า** (`lib/ai.js` + `lib/providers.js`) 10 ตัวเลือก: พื้นฐาน(heuristic ฟรี) + Gemini/Claude/GPT/DeepSeek/Kimi/Qwen/GLM/**Grok/Llama** (2 ตัวหลังเพิ่งเพิ่มโครง 22 ก.ค. รอกุญแจ probe) · ชื่อเจ้า = แบรนด์+(บริษัท) ไม่มี tag · ชื่อรุ่นแยก 🧠Thinking/💬คิดธรรมดา ด้วยรหัสลงท้าย `::min` (GPT→reasoning_effort:none · Gemini→thinkingBudget:0) · ดาวใช้ครึ่งดาวไม่ปัดขึ้น
