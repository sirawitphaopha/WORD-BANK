...
- ตัวจัดคำ = heuristic (`lib/engine.js`) → สลับเป็น Claude API ได้โดยแก้แค่ `runEngine()` คืน response เดิม

## แผนที่ไฟล์ (แก้ตรงไหน)
- แก้ UI/หน้า/ต้นไม้/สารบัญ/settings → `components/WordBankApp.jsx` (class เดียว ทั้งแอป)
- แก้ตัวจัดคำ heuristic → `lib/engine.js` (พอร์ตเป็น Claude ที่นี่)
...
## ฐานข้อมูล
`wb_categories` (id c0..c8) · `wb_novels` · `wb_words`(text, meaning, category_id, **subpath** เส้นทางหมวดย่อย 3 ชั้นคั่น " / ", **highlight** คำเน้น, novel, ...) · RLS deny-all
...
## ถัดไป
ต่อ Claude API จริง · deploy Cloudflare Workers · ระบบไฮไลต์คำเน้น (field highlight) · ระบบล็อกอิน (ถ้าต้องการ)
