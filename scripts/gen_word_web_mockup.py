...
function draw() {
  ...
  const seq = i => 0.34 + i * .03;
  /* จุดเล็กวิ่งไปตามเส้นหนึ่งเที่ยวหลังเส้นวาดเสร็จ — เหมือนน้ำเลี้ยงไหลจากวลีไปหาคำที่แตกออกมา */
  const pulses = [];
  svg.innerHTML = P.map(([d, c, w, o, dash, lk, grp], i) => {
    if (!dash && (lk != null || grp != null)) {
      const leafLine = lk != null;
      const dur = leafLine ? 2.4 : 3.4, bg = (seq(i) + .55 + i * 0.13).toFixed(2);
      pulses.push(
        `<circle r="${leafLine ? 3.4 : 2.6}" fill="${c}" opacity="0" style="filter:drop-shadow(0 0 4px ${c})">`
        + `<animateMotion dur="${dur}s" begin="${bg}s" repeatCount="indefinite"><mpath href="#ln${i}"/></animateMotion>`
        + `<animate attributeName="opacity" values="0;${leafLine ? '.95;.95' : '.7;.7'};0" keyTimes="0;.1;.8;1"`
        + ` dur="${dur}s" begin="${bg}s" repeatCount="indefinite"/></circle>`);
    }
    ...
    + dots.map(([x, y, c, r, o], i) => `<circle cx="${f(x)}" cy="${f(y)}" r="${r}" fill="${c}" opacity="${o}"`
      + ` style="transform-box:fill-box;transform-origin:center;animation:dotIn .34s cubic-bezier(.2,.9,.3,1) ${(0.5 + i * .022).toFixed(2)}s both"/>`).join('')
    + pulses.join('');
