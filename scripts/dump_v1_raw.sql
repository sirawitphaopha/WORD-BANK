...
--  • ท้ายผลมี review_batch_summary = รายชื่อทุกช่อที่ยังเหลือใน wb_review ตอนนี้ (ใช้ดูว่าช่อไหนจริง/ช่อไหน junk ที่อยากลบ)

select json_build_object(
  'v1_review_words', (
    select coalesce(json_agg(row_to_json(w) order by w.batch, w.position), '[]'::json)
    from (
  ...
  'review_batch_summary', (
    select coalesce(json_agg(row_to_json(s) order by s.batch), '[]'::json)
    from (
      select batch, batch_ai, novel, count(*) as n
      from wb_review
      group by batch, batch_ai, novel
    ) s
  )
) as v1_dump;
