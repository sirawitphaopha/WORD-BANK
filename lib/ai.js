...
// 4 ประเภทที่เจ้าของคลังเคาะ 3 ส.ค. 2569 — ตัดคำมูลกับคำสมาสออก เพราะไม่ช่วยงานเขียนนิยาย
const WORD_FORMS = ['คำซ้อน', 'คำซ้ำ', 'คำประสม', 'คำทับศัพท์'];
...
function normalize(raw, categories) {
  ...
  for (const it of items) {
    ...
    // word_form — คำนี้สร้างขึ้นยังไง (คนละแกนกับ kind ที่บอกว่ายาวแค่ไหน)
    // ค่านอกรายการ = ทิ้ง ปล่อยว่าง · ว่างแปลว่า "ยังไม่ได้ดู" ไม่ใช่ "ผิด"
    const wordForm = WORD_FORMS.includes(it.word_form) ? it.word_form : null;
    ...
    const subs = [...new Set([...rawSubs, it.subcategory]
      ...
      .filter(Boolean))];
    // ความหมาย — รับได้ทั้ง meanings (อาร์เรย์ ของใหม่ ฉบับที่ ๑๑) และ meaning (ข้อความเดียว ของเดิม)
    const mnList = [...new Set((Array.isArray(it.meanings) ? it.meanings
      : (it.meaning != null && String(it.meaning).trim() ? [it.meaning] : []))
      .map((s) => String(s || '').trim())
      .filter(Boolean))];
    ...
    out.push({
      ...
      // 🌿 คำสั่งฉบับที่ ๑๑ — ความหมายมาเป็น "รายการ" (1 ช่องต่อ 1 ความหมาย) และไม่จำกัดความยาว
      //    (พี่กันสั่ง 27 ก.ค. 2569 "ถ้าวลีนั้นมี 6 ความหมายมันก็ต้องยาวได้สิ ให้ใส่แบบบุลเล็ต")
      //    เก็บรายการไว้ใน meanings สำหรับฐานข้อมูล + ต่อเป็นข้อความเดียวใส่ meaning ให้หน้าเว็บเดิมใช้ได้เหมือนเดิม
      meanings: mnList,
      meaning: mnList.length ? mnList.join(' · ') : '',
...
async function callOpenAICompatible(cfg, apiKey, text, categories, system, live) {
  // 🧠 โหมดคิด (thinking/ธรรมดา · roadmap P): id ลงท้าย "::min/low/high" = รุ่นเดิม + คุม reasoning_effort
  //    "::min" = โหมดธรรมดา ตอบเร็ว ไม่คิด (เทสแล้ว GPT-5.x รับ 'none' · 'minimal' ใช้ไม่ได้) · ไม่มี suffix = คิดดีฟอลต์ (เหมือนเทส V2)
  //    ค่าที่ GPT-5.x รองรับ: none/low/medium/high · เจ้าอื่นไม่มี suffix จึงไม่ส่ง reasoning_effort
  let apiModel = cfg.model || '', effort = null;
  const mm = /^(.+)::(none|min|low|medium|high)$/.exec(apiModel);
  if (mm) { apiModel = mm[1]; effort = mm[2] === 'min' ? 'none' : mm[2]; }
  const body = {
    model: apiModel,
    messages: [
  ...
  if (!/^gpt-5/.test(apiModel)) body.temperature = 0.2;
...
async function callGemini(cfg, apiKey, text, categories, system, live) {
  // 🧠 โหมดคิด: id ลงท้าย "::min" = ปิดคิด (thinkingBudget 0 = ธรรมดา) · ไม่มี suffix = คิดดีฟอลต์ (เหมือนเทส V2)
  //   ปิดคิดได้เฉพาะรุ่นที่รองรับ (3.5-flash/3-flash/3.1-flash-lite เทสแล้ว) · Pro/3.6-flash/3.5-flash-lite ปิดไม่ได้ (คิดอย่างเดียว)
  let apiModel = cfg.model || '';
  const genCfg = { responseMimeType: 'application/json', temperature: 0.2 };
  const mm = /^(.+)::min$/.exec(apiModel);
  if (mm) { apiModel = mm[1]; genCfg.thinkingConfig = { thinkingBudget: 0 }; }
...
async function callClaude(cfg, apiKey, text, categories, system, live) {
  ...
  // แยก "ระดับความคิด" (effort) จากรหัสรุ่น เช่น claude-opus-4-8::max → effort 'max'
  // Claude Opus 4.8/4.7 คุมความลึกการคิดด้วย output_config.effort (low/medium/high/xhigh/max) + คิดแบบ adaptive
  //   (ต่างจาก GPT/Gemini · ยืนยันกับ API จริงแล้ว 22 ก.ค. 2569 · thinking.type:'enabled' เด้ง error ต้องใช้ effort)
  //   ไม่มี ::<effort> = ไม่ส่ง output_config = ใช้ค่าเริ่มต้นของ API (effort high)
  let apiModel = cfg.model || '', effort = null;
  const mm = /^(.+)::(low|medium|high|xhigh|max)$/.exec(apiModel);
  if (mm) { apiModel = mm[1]; effort = mm[2]; }
  const body = {
    model: apiModel,
    max_tokens: 4096,
    system,
...
// export ไว้ให้เทสเรียกได้ (เป็น util ล้วน ไม่มี side-effect)
export function restoreMissingLines(text, items, categories) {
  const lines = String(text || '').split('\n').map((s) => s.trim()).filter(Boolean);
  if (!lines.length) return { items, restored: [] };

  // คำที่ AI คืนมา "ในฐานะคำเดี่ยว/ประโยคเต็ม" (source ว่าง) เก็บเป็นกุญแจไว้เทียบ
  // 🛟 นับเฉพาะ source ว่าง ไม่นับคำที่ AI สกัดจากประโยคแม่ (source มีค่า) — เพราะบรรทัดที่ผู้ใช้พิมพ์
  //    ต้องอยู่เป็น "คำของตัวเอง" ถ้าคำนั้นโผล่แค่ในฐานะคำสกัดของประโยคอื่น = ยังถือว่าหาย ต้องเติมกลับ
  //    (บั๊กเดิมนับคำสกัดด้วย → ตระกูล GPT ที่กลืนคำเดี่ยวไปเป็นคำสกัดจึงไม่ถูกเติมกลับ = H4 เวอร์ชัน 2 ไม่ผ่าน)
  const have = new Set(
    items
      .filter((it) => !(it && it.source != null && String(it.source).trim()))
      .map((it) => lineKey(it && it.text))
      .filter(Boolean),
  );
  const restored = [];
