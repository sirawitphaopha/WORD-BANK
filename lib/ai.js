...
export async function runAI({ text, categories, provider, model, prompt }) {
  ...
  let result;
  if (cfg.kind === 'openai') result = await callOpenAICompatible(cfg, apiKey, text, categories, system);
  else if (cfg.kind === 'gemini') result = await callGemini(cfg, apiKey, text, categories, system);
  else if (cfg.kind === 'claude') result = await callClaude(cfg, apiKey, text, categories, system);
  else result = { ...runEngine(text), usage: { in: 0, out: 0 } };
  return { ...result, model: useModel, provider: key };
}
