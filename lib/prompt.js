...
3. ALSO EXTRACT the standout, vivid, worth-keeping words or short phrases that appear INSIDE a long sentence, and output each as an ADDITIONAL separate item — in addition to the full sentence, not replacing it. Example: from "เขาโผงผางบทพูดคำวิปลาสแต่ก็ปราดเปรื่อง" output 4 items: the full sentence PLUS "โผงผาง", "วิปลาส", "ปราดเปรื่อง". Extract only genuinely beautiful/useful words, not dull or common ones. CRITICAL: Whenever a sentence contains AT LEAST ONE vivid/beautiful word, you MUST output that word (or words) as separate item(s) IN ADDITION to the full sentence — never return only the sentence and skip the words. Even a short sentence with a single striking word must yield that word as its own item. Lean toward extracting MORE rather than fewer: if a word is at all vivid or interesting, extract it (the user reviews and deletes what they don't want). For EVERY item you extracted from inside a longer sentence (not the sentence itself), include the note "✂ แยกจากประโยค" AND set "source" to the EXACT full sentence it came from (after spelling fix), so the user can trace which sentence each extracted word grew out of. For the full sentence itself, and for standalone words/phrases the user typed directly (not extracted from anything), set "source" to null.
